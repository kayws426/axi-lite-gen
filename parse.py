...
C_S_AXI_ADDR_MSB = C_S_AXI_ADDR_WIDTH-1
C_S_AXI_ADDR_LSB = 2
...
class Value():
    ...
    def __init__(self, word_index=None, bits=None, section=None, slug=None,
        ...
        assert(self.index >= 0)
        assert(self.index <= (2**C_S_AXI_ADDR_WIDTH - 1))
...
def check_overlaps(l):
    ...
    for val in l:
        ...
        this = (val.index, val.index + ((val.bits-1)/C_S_AXI_DATA_WIDTH))
        inserted = False
...
def output(registers, parameters, sections):
    ...
    context = dict(registers=registers,
                ...
                whoami=os.getenv('USER') or os.getenv('USERNAME'),
                sections=sections,
